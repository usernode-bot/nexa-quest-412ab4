...
async function migrate(pool) {
  ...
  `);
  // Read by a 24-hour window (the alert evaluator) and a 30-day one (the
  // dashboard's acceptance rate). This table is never pruned, so without an
  // index those two windowed scans grow with every consent dialog ever shown,
  // on a table staging copies whole.
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_grant_events_created ON llm_grant_events (created_at DESC)`);
  ...
  // The dashboard reads this table by TIME WINDOW, not by room, so the
  // room-leading index above cannot serve it: a 7-day window matches nearly
  // every row, which made Postgres sequential-scan the whole table on every
  // /admin/metrics load. Two percentile aggregates over it are the slowest
  // pair of queries the app issues, and with ten of them fanned out per view
  // they could exceed the pool's statement_timeout and 500 the page. This
  // plain created_at index is what the windowed aggregates can actually use.
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_latency_created ON latency_samples (created_at DESC)`);
  ...
  // Cost, sessions and the degradation mix, all read by window across every
  // room. Same reasoning as idx_latency_created: the room-leading index is
  // for one room's history, not for "the last seven days, everywhere".
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_room_sessions_started ON room_sessions (started_at DESC)`);
