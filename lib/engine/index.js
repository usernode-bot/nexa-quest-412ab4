...
// A speaker changed the language they speak: their translation sessions are
// stale everywhere, not just on the active engine.
function resetSpeaker(roomId, speakerUserId) {
  for (const e of ENGINES) if (e.resetSpeaker) e.resetSpeaker(roomId, speakerUserId);
}

function sweepSessions(idleMs) {
  let dropped = 0;
  for (const e of ENGINES) if (e.sweepSessions) dropped += e.sweepSessions(idleMs);
  return dropped;
}

function queueDepth() {
  let n = 0;
  for (const e of ENGINES) if (e.queueDepth) n += e.queueDepth();
  return n;
}

// Fan-out queues for rooms that no longer have a live translation session.
// Called from housekeeping; the Maps are process-local, so nothing else will
// ever clean them up.
function sweepQueues() {
  let swept = 0;
  for (const e of ENGINES) if (e.sweepQueues) swept += e.sweepQueues();
  return swept;
}

function sessionCount() {
  let n = 0;
  for (const e of ENGINES) if (e.sessionCount) n += e.sessionCount();
  return n;
}
