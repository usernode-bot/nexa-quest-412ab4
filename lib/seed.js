...
async function seedStaging(pool) {
  ...
  try {
    ...
    // --- Stage 5/6 surfaces ------------------------------------------------
    // Two translations that FAILED, written as new rows rather than by
    // mutating the successful ones above. The failure panel on /admin/metrics
    // groups by `fail_code`, and an empty group is indistinguishable from a
    // panel that is broken.
    const FAILED = [
      [900461, 900301, 'ja', 'unavailable', 'grant_required'],
      [900462, 900302, 'es', 'error', 'timeout'],
    ];
    for (const [rowId, uttId, target, status, code] of FAILED) {
      await client.query(
        `INSERT INTO utterance_translations
           (id, utterance_id, room_id, target_lang, status, fail_code, engine_id, seq, created_at)
         SELECT $1, u.id, u.room_id, $3, $4, $5, 'proxy-text', u.seq, NOW() - INTERVAL '30 minutes'
           FROM utterances u WHERE u.id = $2
         ON CONFLICT (utterance_id, target_lang) DO NOTHING`,
        [rowId, uttId, target, status, code]
      );
    }

    // Client and server error events. Messages are diagnostics, never caption
    // text: an error report describes the browser, not the conversation.
    const ERRORS = [
      [900701, 'client', 'poll_failed', 'stream', 'NetworkError when attempting to fetch resource', 2],
      [900702, 'client', 'audio_start_timeout', 'audio', 'synthesis did not start within 2500ms', 5],
      [900703, 'client', 'audio_watchdog', 'audio', 'utterance stalled past the watchdog window', 9],
      [900704, 'server', 'internal', '/api/rooms/DEMOSUP/utterances', 'connection terminated unexpectedly', 14],
      [900705, 'client', 'render_failed', 'metrics', 'Cannot read properties of null', 20],
      [900706, 'client', 'boot_failed', 'boot', 'Failed to fetch /api/config', 26],
      [900707, 'server', 'internal', '/api/rooms/DEMOHALL/stream', 'statement timeout', 33],
      [900708, 'client', 'flush_failed', 'outbox', 'HTTP 503 from /api/rooms/DEMOTEAM/utterances', 41],
    ];
    for (const [id, source, code, where, message, hoursAgo] of ERRORS) {
      await client.query(
        `INSERT INTO error_events (id, source, code, where_at, message, user_agent, room_id, req_id, created_at)
         VALUES ($1,$2,$3,$4,$5,'Staging demo browser', $6, $7, NOW() - ($8 || ' hours')::INTERVAL)
         ON CONFLICT (id) DO NOTHING`,
        [id, source, code, where, `${message} [staging demo]`,
          source === 'server' ? SUP : null, `demo${id}`.slice(-8), hoursAgo]
      );
    }

    // Alerts: one open warning, one open critical, one already resolved, so
    // the alert list has all three states to render.
    const ALERTS = [
      [900901, 'first_word_p95', 'warn', 3, null, { valueMs: 612, targetMs: 500, n: 184 }, null],
      [900902, 'audio_fallback_rate', 'crit', 1, null, { rate: 0.43, target: 0.2, n: 96 }, HALL],
      [900903, 'error_rate', 'warn', 11, 9, { rate: 0.031, target: 0.02, n: 220 }, null],
    ];
    for (const [id, rule, severity, openedHrs, resolvedHrs, detail, roomId] of ALERTS) {
      await client.query(
        `INSERT INTO alerts (id, rule, severity, opened_at, resolved_at, detail, room_id)
         VALUES ($1,$2,$3, NOW() - ($4 || ' hours')::INTERVAL,
                 CASE WHEN $5::int IS NULL THEN NULL ELSE NOW() - ($5 || ' hours')::INTERVAL END,
                 $6::jsonb, $7)
         ON CONFLICT (id) DO NOTHING`,
        [id, rule, severity, openedHrs, resolvedHrs, JSON.stringify(detail), roomId]
      );
    }

    // Pilot feedback from the fake testers, one per pinned use case plus two
    // more. Never the visitor: /admin/feedback and /feedback must answer the
    // same way for a real user in production as they do in a preview, and the
    // "Translation preview" test exists to pin exactly that.
    const FEEDBACK = [
      [900801, PEOPLE.anin, SUP, 'onboarding', 5, 'Akhirnya bisa dipandu setup node pakai bahasa sendiri.', true,
        { speech: true, mic: true, tts: true, voices: 2, longPoll: true, platform: 'android' }],
      [900802, PEOPLE.mark, SUP, 'support', 4, 'Captions arrive fast enough to answer without waiting.', true,
        { speech: true, mic: true, tts: true, voices: 3, longPoll: true, platform: 'desktop' }],
      [900803, PEOPLE.rina, TEAM, 'validator', 4, 'Tiga orang bicara bergantian masih terbaca rapi.', false,
        { speech: true, mic: true, tts: true, voices: 1, longPoll: true, platform: 'desktop' }],
      [900804, PEOPLE.budi, HALL, 'townhall', 3, 'Suaranya bagus, tapi mikrofon tidak bisa diakses di aplikasi.', false,
        // The microphone finding, recorded as a real device report.
        { speech: true, mic: false, micPolicy: 'undelegated', tts: true, voices: 2, longPoll: false, platform: 'ios' }],
      [900805, PEOPLE.yuki, HALL, 'townhall', 5, '字幕の遅れはほとんど気になりませんでした。', false,
        { speech: false, mic: true, tts: true, voices: 4, longPoll: false, platform: 'desktop' }],
      [900806, PEOPLE.tom, TEAM, 'validator', 2, 'No Indonesian voice on this device, so I read instead of listened.', true,
        { speech: true, mic: true, tts: true, voices: 0, longPoll: true, platform: 'desktop' }],
    ];
    for (const [id, person, roomId, purpose, rating, comment, contactOk, compat] of FEEDBACK) {
      await client.query(
        `INSERT INTO pilot_feedback
           (id, user_id, username, room_id, purpose, rating, comment, contact_ok, compat, created_at)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb, NOW() - ($10 || ' hours')::INTERVAL)
         ON CONFLICT (id) DO NOTHING`,
        [id, person.id, person.username, roomId, purpose, rating,
          `${comment} [staging demo]`, contactOk, JSON.stringify(compat), (id - 900800) * 6]
      );
    }

    await client.query('COMMIT');
    log.info('seed', { msg: 'staging demo data ready (DEMOSUP, DEMOTEAM, DEMOHALL)' });
  } catch (err) {
    await client.query('ROLLBACK');
    log.error('seed', { msg: err.message });
  } finally {
    client.release();
  }
}
