...
//
// The query is shared per room, which is the part that decides whether this
// scales. Held listeners in the same room all want the answer to the identical
// question ("has seq moved?"), so one watcher per room asks it once per tick
// and every waiter reads the same answer. Per-listener polling made a fifty
// person townhall issue two hundred `SELECT seq` per second against one row;
// the shared watcher makes it four, whatever the audience size. External
// behaviour is unchanged: the same hold, the same tick, the same abandonment
// the moment a drain starts.
const watchers = new Map();

function watcherFor(pool, roomId, tickMs) {
  const key = String(roomId);
  let w = watchers.get(key);
  if (w) return w;
  w = { seq: null, ended: false, gone: false, waiters: new Set(), timer: null, tickMs };
  watchers.set(key, w);

  const tick = async () => {
    // Nobody left waiting: stop asking. The next held request starts a fresh
    // watcher, so a quiet room costs nothing between calls.
    if (!w.waiters.size) { watchers.delete(key); w.timer = null; return; }
    try {
      const r = await pool.query('SELECT seq, ended_at FROM rooms WHERE id = $1', [roomId]);
      const row = r.rows[0];
      if (!row) w.gone = true;
      else { w.seq = Number(row.seq); w.ended = !!row.ended_at; }
    } catch {
      // A failed probe is not an answer. Wake everyone so they reply with
      // whatever they already have rather than holding through an outage.
      w.failed = true;
    }
    const woken = Array.from(w.waiters);
    w.waiters.clear();
    for (const wake of woken) wake();
    if (w.waiters.size === 0 && !watchers.has(key)) return;
    w.timer = setTimeout(tick, w.tickMs);
    w.timer.unref?.();
  };
...
function once(w) {
  return new Promise((resolve) => { w.waiters.add(resolve); });
}
...
async function waitForChange(pool, roomId, since, opts) {
  ...
  while (Date.now() < deadline) {
    ...
    const w = watcherFor(pool, roomId, tickMs);
    // Race the shared tick against the caller's own remaining hold, so a
    // request that is nearly out of time is not held for a whole extra tick.
    await Promise.race([once(w), sleep(Math.max(0, deadline - Date.now()))]);
    if (isShuttingDown()) return false;
    if (w.failed) return false;                  // answer with whatever we have
    if (w.gone) return true;                     // room vanished: let the caller say so
    if (w.ended) return true;                    // ending is a change worth shipping
    if (w.seq !== null && w.seq > since) return true;
  }
  return false;
}

// Watchers hold nothing but a timer and a set of resolvers, and they retire
// themselves on the first tick with no waiters. Exposed for the shutdown path,
// which would otherwise let a pending tick fire against a closed pool.
...
async function assemble(deps, args) {
  ...
  return {
    ...
    dialIn: telephony.status(deps.uiLang),
...
module.exports = { assemble, waitForChange, buildEvents, stopWatchers };
